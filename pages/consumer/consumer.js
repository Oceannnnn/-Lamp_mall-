...
Page({
  ...
  receiveBonus(e) {//自己拆红包
    ...
    let bonus = wx.getStorageSync('bonus').bonus;
    util.http('bonus/getBonus', { id: redrnvelopes }, 'post', token).then(res => {
      if (res.code == 200) {
        this.setData({
          isGetBonus: 1//领取过
        })
  ...
  redrnvelopesList(bonus,id,token) {
    util.http('bonus/getBonusInfo', { bonus:bonus,id: id }, 'post', token).then(res => {
      if (res.code == 200) {
        let info = res.data;
        this.setData({
  ...
  pullTogether(e) {//一起拆红包
    ...
    if (newText) {
      ...
      util.http('bonus/pullTogether', { id: id }, 'post', token).then(res => {
        if (res.code == 200) {
          let info = res.data
          this.setData({
            redrnvelopesList: info.avatar,
  ...
  pullBonus(){//立即拆红包，人数已够
    ...
    let bonus = wx.getStorageSync('bonus').bonus;
    let id = this.data.bonusID;
    util.http('bonus/pullBonus', { id: id }, 'post', token).then(res => {
      if (res.code == 200) {
        this.redrnvelopesList(bonus, id, token)
        main.member();
      }
  ...
  onReachBottom() {
    ...
    if (this.data.onBottom) {
      this.couponList(this.data.page, this.data.currentId);
    }
  },
  couponList(page, currentId) {
    ...
    util.http('index/coupon', json, 'post', token).then(res => {
      if (res.code == 200) {
        if (currentId == 1) {
          name = res.data.money_off
        }else{
          name = res.data.discount
        }
        if (name != '') {
