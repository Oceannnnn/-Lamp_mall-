...
Page({
  ...
  comfirmBtn(e, priceId, id) {
    ...
    if (comfirmistuan == 1) { //拼团
      ...
      util.http('tuan/add', json, 'post', token).then(res => {
        if (res.code == 200) {
      ...
      if (comfirmCart == 1) {
        ...
        util.http('order/add', { products: jsonArr }, 'post', token).then(res => {
          if (res.code == 200) {
            wx.removeStorageSync('orderData');
            wx.setStorage({
              key: "orderData",
  ...
  onShareAppMessage(ops) {
    ...
    return {
      ...
      path: url + '&invite_code=' + invite_code
    }
  },
})
